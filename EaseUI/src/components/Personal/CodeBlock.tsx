...
const CodeBlock = ({ code, language = "tsx" }: CodeBlockProps) => {
  ...
  return (
    <div className="relative">
      ...
      <pre className="border border-t-0 p-4 rounded-b-md overflow-x-auto" style={{ borderColor: 'var(--card-bg)', backgroundColor: 'var(--bg-color)' }}>
        <code className="text-sm" style={{ color: 'var(--text-color)' }}>{code}</code>
      </pre>
    </div>
  );
};
